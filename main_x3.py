...
class Main(QtWidgets.QWidget, Ui_Form):
    ...
    def check_and_prompt_supply_request(self):
        ...
        Если для какого-либо материала наблюдается недостаток, то:
         - Если остаток поставок (remainder) для записей, связанных с заказом, достаточен – переводим материал из остатка в заказ.
         - Если остаток недостаточен – предлагаем создать заявку на поставку.
        """
        self.shortage_data = {}
        ...
        with self.db_manager as db:
            if not self.shortage_data:
                db.execute_query("UPDATE order_request SET status = %s WHERE id = %s",
                                 ("Готово в цеху", self.current_order['id']))
                ...
                for material, shortage in self.shortage_data.items():
                    result = db.execute_query(
                        """
                        SELECT SUM(sc.remainder) as total_remainder, sc.length
                        FROM supply_composition sc
                        JOIN material m ON sc.material_id = m.id
                        WHERE m.name = %s
                          AND sc.remainder > 0
                    ...
                    if total_remainder == 0:
                        ...
                        shortage_new = shortage * result[0]['length']

                    # total_remainder_new = total_remainder * result[0]['length']
                    # shortage_new = shortage * result[0]['length']
                    # print("601", total_remainder_new, shortage_new)
                    # print("602", total_remainder, shortage)
                    # print(total_remainder < shortage_new)
                    # print("604",self.shortage_data.items())

                    if total_remainder < shortage_new:
                        print("606",total_remainder, shortage)
                        all_available = False
                    ...
                    for material, shortage in self.shortage_data.items():
                        ...
                        for row in rows:
                            ...
                            oc_res = db.execute_query(
                                "SELECT pm.id, oc.id as oc_id FROM order_composition oc JOIN product_materials pm ON oc.id = pm.order_composition_id JOIN supply_composition sc ON pm.supply_composition_id = sc.id JOIN material m ON sc.material_id = m.id WHERE oc.order_id = %s AND m.name = %s AND sc.id = %s LIMIT 1",
                            ...
                            length_supply = db.execute_query("SELECT length FROM `supply_composition` WHERE id = %s",
                                             (row['id']))
                            print(length_supply[0]["length"])
                            print(float(length_supply[0]["length"]))
                            print(transfer_amount)
                            print(int(transfer_amount) * float(length_supply[0]["length"]))

                            transfer_amount_length = int(transfer_amount) * float(length_supply[0]["length"])
                            print(transfer_amount_length)
        ...
        try:
            ...
            with self.db_manager as db:
                for material_name, shortage in self.shortage_data.items():
                    ...
                    oc_res = db.execute_query(
                        "SELECT oc.id FROM order_composition oc join product_materials pm on oc.id = pm.order_composition_id join supply_composition sc on sc.id = pm.supply_composition_id WHERE oc.order_id = %s group by pm.order_composition_id;",
                        (self.current_order['id'],)
                    )
