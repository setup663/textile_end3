...
class Main(QtWidgets.QWidget, Ui_Form):
                    ...
                    if not insufficient_materials:
                        ...
                        for material in sufficient_materials:
                            ...
                            for row in rows:
                                ...
                                db.execute_query(
                                    "UPDATE supply_composition SET remainder = remainder - %s WHERE id = %s",
    ...
    def create_supply_requests(self, materials_to_order):
        ...
        Для каждого материала автоматически подставляются длина и ширина из последней поставки,
        которая связана с текущим заказом через product_materials и order_composition.
        ...
        try:
            with self.db_manager as db:
                for material_name, shortage in materials_to_order.items():
                    ...
                    # Получаем последние значения длины и ширины для данного материала,
                    # используя связь через product_materials и order_composition для текущего заказа.
                    last_supply = db.execute_query(
                        """
                        SELECT sc.length, sc.width 
                        FROM supply_composition sc
                        JOIN product_materials pm ON sc.id = pm.supply_composition_id
                        JOIN order_composition oc ON pm.order_composition_id = oc.id
                        WHERE sc.material_id = %s AND oc.order_id = %s
                        ORDER BY sc.id DESC LIMIT 1
                        """,
                        (material_id, self.current_order['id'])
                    )
                    if last_supply:
                        length = float(last_supply[0]['length'])
                        width = float(last_supply[0]['width'])
                    else:
                        self.show_error_message(
                            f"Нет предыдущей поставки для материала {material_name} в рамках этого заказа.")
                        continue
